...
MAX_CLIENTS_PER_PORT_HARD_LIMIT = 5
...
def get_random_port():
    ...
    return random.choice(available) if available else random.choice(all_ports)
...
def standby_producer_thread():
    ...
    while not stop:
        try:
            needs_standby = standby_tokens.qsize() < STANDBY_MAX_PER_CHANNEL
            ...
            if needs_standby and pool_healthy:
                ...
                if port:
                    ...
                    if token:
                        ...
                        logger.debug(f"[STANDBY] produced token on port:{port} | pool: {standby_tokens.qsize()}/{STANDBY_MAX_PER_CHANNEL}")
                    else:
...
def show_stats():
    ...
    while not stop:
        try:
            ...
            print("\033[7A", end="")
            print(f"\033[2K\r[+] Containers: \033[32m{len(containers)}\033[0m | Ports: \033[32m{len(all_ports)}\033[0m | Version: \033[94mv{VERSION}\033[0m")
            print(f"\033[2K\r[+] Port Health: \033[32m{healthy_ports}/{len(all_ports)}\033[0m good | Standby: \033[36m{standby_tokens.qsize()}/{STANDBY_MAX_PER_CHANNEL}\033[0m")
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            while not stop:
                ...
                for _ in range(batch_size):
                    ...
                    await asyncio.sleep(random.uniform(MIN_SPAWN_DELAY, MAX_SPAWN_DELAY) if batch_size > 1 else 0.05)
...
if __name__ == "__main__":
    try:
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"=== Kick Viewer Bot (Multi-Tor Docker 3-Port) v{VERSION} ===\n")
        ...
        # Channel selection: prefer liste.txt, fall back to prompt
        channel_input = ""
        liste_entries = read_liste()
        if liste_entries:
            print(f"\n[*] Found {len(liste_entries)} channel(s) in {LIST_FILE}:")
            for i, e in enumerate(liste_entries, 1):
                print(f"  {i}. {e}")
            choice = input(f"Select channel number (or press Enter to use channel 1): ").strip()
            if choice.isdigit() and 1 <= int(choice) <= len(liste_entries):
                channel_input = liste_entries[int(choice) - 1]
            else:
                channel_input = liste_entries[0]
            print(f"[*] Using channel: {channel_input}")
        else:
            channel_input = input("\nChannel: ").strip()
